...
class Retriever:
    ...
    def retrieve(
        ...
        #
        # 闸门判的是「有没有锚点」，所以判成布尔，不比分数。旧写法拿
        # _score(profile=None) 的整体加权分跟 MIN_RETRIEVAL_SCORE=0.12 比，其中
        # keyword 项占 0.30、无画像时分母是 0.9，于是同一个「熔断器 ↔ 熔断」锚点：
        #   命中 1/1 → keyword=1.00 → 0.30×1.00/0.9 = 0.333 ≥ 0.12 → 出候选
        #   命中 1/3 → keyword=0.33 → 0.30×0.33/0.9 = 0.111 < 0.12 → 整片空白
        # 也就是让「模型这次抽了几个词」决定检索出不出结果，线上表现为随机空白。
