...
function openNoteSheet(noteId = null, template = null) { const note = getNotes().find((item) => item.id === noteId); editingNoteId = note?.id || makeId("note"); pendingNoteTemplate = clone(note?.template || template || defaultNoteTemplate()); pendingNoteCoverFile = null; pendingNoteCoverUrl = note?.coverUrl || ""; const drafts = attachmentDraftMap()[editingNoteId] || []; pendingAttachments = clone((note?.attachments || drafts).filter((file) => !file.noteId || file.noteId === editingNoteId).map((file) => ({ ...file, noteId: editingNoteId }))); $("noteSheetTitle").textContent = note ? "编辑笔记" : "新建笔记"; $("noteTitle").value = note?.title || ""; $("noteBody").innerHTML = note?.content || ""; applyNoteTemplate(pendingNoteTemplate); const select = $("noteCategorySelect"); select.replaceChildren(); select.add(new Option("根目录", "")); getCategories().forEach((category) => select.add(new Option(category.name, category.id))); select.add(new Option("＋ 新建目录", "__new_category__")); select.value = noteFolderId(note || {}) || (currentCategory === "all" ? "" : currentCategory); renderAttachments(); $("deleteNote").hidden = !note; $("sheetBackdrop").hidden = false; $("noteSheet").hidden = false; document.body.classList.add("sheet-open"); $("noteTitle").focus(); }
...
$("noteCategorySelect").addEventListener("change", (event) => {
  if (event.target.value !== "__new_category__") return;
  event.target.value = currentCategory === "all" ? "" : currentCategory;
  openFolderSheet();
});
